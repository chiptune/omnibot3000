...
const useChatCompletionStore = create<ChatCompletionStoreState>()((set, get) => ({
  ...
  getChats: () => get().chats,
  getChat: (id?: ChatId) => get().chats.find((chat: Chat) => Boolean(id === chat.id)),
  ...
  updateChatTitle: (id: ChatId, title: string) => {
    const updatedChats: Chat[] = get().chats.map((chat: Chat): Chat => (chat.id === id ? {...chat, title} : chat));
    set({chats: updatedChats});
  },
  completions: [],
  ...
  getCompletion: (id: CompletionId) => get().completions.find((completion: Completion) => completion.id === id),
  updateChatCompletions: (id: ChatId) => {
    const updatedChats: Chat[] = get().chats.map(
      (chat: Chat): Chat => (chat.id === id ? {...chat, completions: get().completions} : chat),
    );
    set({chats: updatedChats});
  },
  resetCompletions: () =>
